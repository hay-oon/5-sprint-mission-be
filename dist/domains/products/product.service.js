...
const createProduct = async (name, description, price, tags, images, userId) => {
    ...
    // 응답 객체 생성 및 반환
    return (0, formatters_1.createProductResponseDto)(product, user, false);
};
...
const getProducts = async (page = 1, pageSize = 10, orderBy = "recent", keyword, userId = null) => {
    ...
    productsWithDetails = products.map((product) => {
        ...
        const ownerNickname = product.userId &&
            product.userId !== "undefined" &&
            userMap[product.userId]
            ? userMap[product.userId].nickname
            : "Unknown User";
        const ownerInfo = product.userId && userMap[product.userId]
            ? userMap[product.userId]
            : { id: product.userId, nickname: ownerNickname };
        return (0, formatters_1.createProductResponseDto)(product, ownerInfo, false);
    });
    ...
    if (userId) {
        ...
        productsWithDetails = (0, formatters_1.attachFavoriteStatus)(productsWithDetails, favoriteProductIds);
    }
...
const getProductById = async (id, userId = null) => {
    ...
    // 응답 객체 생성 및 반환
    return (0, formatters_1.createProductResponseDto)(product, ownerInfo, isFavorite);
};
...
const updateProduct = async (id, name, description, price, tags, images, userId) => {
    ...
    const product = await prisma.product.update({
        ...
        data: {
            ...
            userId: productCheck.userId === "undefined" ? userId : productCheck.userId,
        },
    });
    // 사용자 정보 조회
    const user = await prisma.user.findUnique({
...
const deleteProduct = async (id, userId) => {
    ...
    // 응답 객체 생성 및 반환
    return (0, formatters_1.createProductResponseDto)(product, ownerInfo, false); // 삭제된 상품이므로 좋아요 상태는 항상 false
};
...
const addFavorite = async (productId, userId) => {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        return (0, formatters_1.createProductResponseDto)(updatedProduct, ownerInfo, true);
    });
};
...
const removeFavorite = async (productId, userId) => {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        return (0, formatters_1.createProductResponseDto)(updatedProduct, ownerInfo, false);
    });
};
