...
const getArticles = async (
  ...
  // 검색 조건 생성
  const where: Prisma.ArticleWhereInput = createSearchCondition(keyword, [
    "title",
    "content",
  ]);
  ...
  if (userId) {
    ...
    const favoriteArticleIds = new Set(
      favorites.map((fav) => fav.articleId).filter(Boolean)
    );
    formattedArticles = attachFavoriteStatus(
      formattedArticles,
      favoriteArticleIds
    );
  }
...
// 게시글 상세 조회
const getArticleById = async (id: string, userId: string | null = null) => {
...
// 게시글에 좋아요 추가
const addFavorite = async (articleId: string, userId: string) => {
  // 트랜잭션 사용
...
// 게시글 좋아요 취소
const removeFavorite = async (articleId: string, userId: string) => {
  // 트랜잭션 사용
  return await prisma.$transaction(async (tx) => {
    // 게시글 존재 여부 확인
    const article = await tx.article.findUnique({
...
const checkFavorite = async (articleId: string, userId: string) => {
  const favorite = await prisma.favorite.findFirst({
    where: {
      userId,
      articleId,
